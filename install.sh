...
# This script will install the latest release of Sndmail.

show_help() {
    cat <<EOF
Sndmail install script
...
Options:
  ...
  -n, --no-sendmail                     Do not link sndmail to /usr/sbin/sendmail
...
v[0-9][0-9\.]*) ;;
*)
    echo "Unable to determine the latest version of Sndmail."
...
GH_REPO_BIN="sndmail-${OS}-${OS_ARCH}.tar.gz"
if [ "$INSTALL_PATH" = "/" ]; then
    INSTALL_BIN_PATH="/sndmail"
else
    INSTALL_BIN_PATH="${INSTALL_PATH}/sndmail"
fi
...
if [ $EXIT_CODE -eq 0 ]; then
    ...
    if [ $EXIT_CODE -eq 0 ]; then
        cp sndmail "$INSTALL_BIN_PATH"
        EXIT_CODE=$?
        if [ $EXIT_CODE -ne 0 ]; then
            echo "ERROR: Copying Sndmail to \"${INSTALL_PATH}\" directory."
        fi
    fi
    ...
    if [ $EXIT_CODE -eq 0 ] && [ "$(id -u)" -eq "0" ]; then
        if [ $NO_SENDMAIL -eq 0 ]; then
            ...
            if [ $EXIT_CODE -ne 0 ]; then
                echo "ERROR: Linking \"${INSTALL_BIN_PATH}\" to \"/usr/sbin/sendmail\"."
            fi
        fi
        OWNER="root"
        GROUP="root"
...
if [ $EXIT_CODE -ne 0 ]; then
    echo "There was an error installing Sndmail."
    exit $EXIT_CODE
fi

echo "Sndmail ${VERSION} installed successfully to \"$INSTALL_BIN_PATH\"."
exit 0
